class Solution:
    def findMin(self, nums: List[int]) -> int:
        ...
        while low< high: #here we use low < high because there is guarantee that the minimum exist. But in binary search we use low<=high because the element might also not be present
            mid=low+((high-low)//2
            )
